...
async function listAttendance(req, res) {
  try {
    ...
    const total = await queryOne(`SELECT COUNT(*) as count FROM attendance a WHERE ${whereClause}`, params);
...
async function clockIn(req, res) {
  try {
    ...
    const existing = await queryOne('SELECT id FROM attendance WHERE staff_id = ? AND date = ? AND clock_out IS NULL', [req.user.id, date]);
    if (existing) return res.status(400).json({ error: 'Already clocked in for this date.' });

    const result = await run('INSERT INTO attendance (shift_id, staff_id, clock_in, date) VALUES (?,?,?,?)',
      [shift_id || null, req.user.id, new Date().toISOString(), date]);

    const record = await queryOne('SELECT * FROM attendance WHERE id = ?', [result.insertId]);
...
async function clockOut(req, res) {
  try {
    ...
    const record = await queryOne('SELECT * FROM attendance WHERE staff_id = ? AND date = ? AND clock_out IS NULL', [req.user.id, date]);
    ...
    await run('UPDATE attendance SET clock_out=?, hours_worked=?, status=? WHERE id=?',
      [now.toISOString(), hoursWorked, hoursWorked > 0 ? 'present' : 'absent', record.id]);
